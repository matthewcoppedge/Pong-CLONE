...
Pong::Application::Application() : m_pong(sf::VideoMode(WIDTH, HEIGHT), "Pong"), m_court_divider(sf::Vector2f(10.f, 10.f)), m_freezeDuration(sf::seconds(6.0f)), m_start_time(0.0f), m_game_over{false} {
    //Load game font
...
void Pong::Application::draw_players_score() {
    ...
    if ((m_tennis_ball.get_x() < -150)) {
        m_SCORE_RIGHT++;
        if (m_SCORE_RIGHT >= 12)  { m_game_over = true; }
        std::stringstream ss;
    ...
    if ((m_tennis_ball.get_x() >  WIDTH + 150)) {
        m_SCORE_LEFT++;
        if (m_SCORE_LEFT >= 12)  { m_game_over = true; }
        std::stringstream ss;
...
void Pong::Application::shader_logic() {
    ...
    float channelTimes[4] = {
        ...
        static_cast<float>(m_clock.getElapsedTime().asSeconds() ) * static_cast<float>(0.5),
    ...
    std::vector<float> iDateValues = {current_Time.asSeconds(), static_cast<float>(current_Time.asMilliseconds()), current_Time.asSeconds() / 60.f, current_Time.asSeconds() / 3600.0f};
    m_shader.setUniformArray("iDateValues", iDateValues.data(), iDateValues.size());
}

void Pong::Application::game_over() {
    sf::Clock game_end;
    [[unlikely]]
    if (m_SCORE_LEFT > 12) {
        sf::Time end_game(sf::seconds(6.0f));
        sf::Text winner;
    ...
    if (m_SCORE_RIGHT > 12) {
        ...
        winner.setString("WINNER");
        winner.setCharacterSize(40);
        winner.setPosition( (WIDTH / 1.5f), (HEIGHT / 2.0f));
        while (end_game > sf::Time::Zero) {
            end_game -= game_end.restart();
            m_pong.clear();
            m_pong.draw(winner);
